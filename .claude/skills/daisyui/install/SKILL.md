...
- CSS собирается standalone-бинарём `tailwindcss-extra` (Tailwind CSS 4 + daisyUI
  внутри самого бинаря, интернет/Node при сборке не нужны). В облачном превью бинарь
  уже установлен; локально Makefile скачивает его в `./bin/` при первой сборке.
...
- **CSS пересобирается сам — руками ничего делать не нужно.** В облачном превью за
  этим следит фоновый watcher: правишь классы в шаблоне → `app.css` обновляется
  автоматически, изменения видны в превью сразу. (Локально то же даёт `make css` или
  `make run`/`build`/`check`, которые собирают CSS перед запуском.) НЕ трать ход на
  `make css` — это лишнее.
